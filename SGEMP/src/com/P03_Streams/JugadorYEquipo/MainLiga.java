...
public class MainLiga {
    public static void main (String[] args) {
        ...
        System.out.println(jugadores.stream().filter(j -> j.getCanastas() > 200 && j.getCanastas() < 500).map(jugador -> jugador.getNombre()).collect(Collectors.toList()));
        // 2.5	Devolver con una lista todos los jugadores ordenados por la fecha de nacimiento.
        System.out.println(jugadores.stream().sorted(comparing(Jugador::getNacim)).collect(Collectors.toList()));
        // 2.6	Devolver con una lista todos los jugadores ordenados por número de canastas.
        System.out.println(jugadores.stream().sorted(comparing(Jugador::getCanastas)).collect(Collectors.toList()));
        // 2.7	Devolver con una lista con los cinco jugadores que hayan realizado más canastas.
        System.out.println(jugadores.stream().sorted(comparing(Jugador::getCanastas)).limit(5).collect(Collectors.toList()));
        ...
        System.out.println(jugadores.stream().filter(j -> j.getCanastas() > 200 && j.getCanastas() < 500).sorted(comparing(Jugador::getCanastas)).collect(Collectors.toList()));
        // 2.9	Devolver con una lista todos los jugadores que satisfacen el filtro del punto 2.3 ordenados, en primer lugar por número de canastas y en segundo lugar (si tienen el mismo número de canastas) por la fecha de nacimiento.
        // 2.10	Devolver el jugador que ha realizado el mínimo número de canastas.
        System.out.println(jugadores.stream().min(Comparator.comparing(Jugador::getCanastas)));
        // 2.11	Devolver el jugador que ha realizado el máximo número de canastas.
        System.out.println(jugadores.stream().max(Comparator.comparing(Jugador::getCanastas)));
        // 2.12	Devolver la media de canastas de todos los jugadores.
        System.out.println(jugadores.stream().mapToDouble(Jugador::getCanastas).average().getAsDouble());
        // 2.13	Devolver con una lista todos los jugadores que pertenezcan a equipos cuya localidad sea Barcelona.
        System.out.println(jugadores.stream().filter(j -> j.getEquipo().getNombre().equals("Barcelona")).collect(Collectors.toList()));
        // 2.14	Devolver con un boolean si hay algún jugador que ha conseguido más de 4.000 canastas
        System.out.println(jugadores.stream().anyMatch(j -> j.getCanastas() > 4000));
        // 2.15	Devolver con un boolean si todos los jugadores han conseguido más de 50 canastas.
        System.out.println(jugadores.stream().allMatch(j -> j.getCanastas() > 50));
        // 2.16	Devolver con un boolean si todos los jugadores del primer equipo añadido han conseguido más de 50 canastas.
        System.out.println(jugadores.stream().filter(j -> j.getEquipo().getNombre().equals("Equipo 1")).allMatch(j -> j.getCanastas() > 50));
